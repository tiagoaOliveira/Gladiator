export function generatePlayerStats(level = 1) {
  ...
  return {
    ...
    attack: Math.floor(15 * Math.pow(1.1, safeLevel - 1)),
    critChance: parseFloat((9.8 + safeLevel * 0.2).toFixed(2)),
    attackSpeed: parseFloat((0.95 + safeLevel * 0.05).toFixed(2)),
    //magicPower: Math.floor(5 * Math.pow(1.1, safeLevel - 1)),
    //magicResistance: Math.floor(5 * Math.pow(1.1, safeLevel - 1)),
    physicalDefense: Math.floor(15 * Math.pow(1.1, safeLevel - 1)),
